...
```diff
+   def is_enhanced_lpo(self, vendor_name, vendor_pn):
+       return based on part number/ vendor
...
+       elif self.is_enhanced_lpo(vendor_name, vendor_pn):
+           xcvr_eeprom = XcvrEeprom(self.reader, self.writer, EnhancedLpoMemMap(CmisCodes))
