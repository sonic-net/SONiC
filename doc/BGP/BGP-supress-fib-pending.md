...
**Note:** After FIB suppression is enabled, but before a config reload is performed for the change to take effect, a lone restart of *fpmsyncd*/*bgp* may cause BGP routes to stop propagating. This does not apply to an *orchagent*/*swss* crash, which cascades a restart of all services so that they re-read the configuration consistently.
