...
* **Cleanup of Unmanaged Users:** Any users found in the Linux system that are not defined in CONFIG_DB (except for system users like `root`, `daemon`, etc.) will be automatically removed to maintain consistency.
...
**Default Admin User (when feature is enabled):**
...
* **`init_cfg.json`:** Contains the feature flag `LOCAL_USER_MANAGEMENT` set to `disabled` by default. No LOCAL_USER entries are included by default to avoid impacting existing deployments.
...
* **To enforce Login Attempts:**
    ...
    * **PAM vs SSH Configuration:** The `pam_faillock` configuration in `/etc/security/faillock.conf` differs significantly from SSH's `MaxAuthTries` in `/etc/ssh/sshd_config`:
      ...
      **How they work together:**
      ...
      - But `pam_faillock` tracks total failures across all connections and methods
      - Once `pam_faillock` threshold is reached, the account is locked even for new SSH connections
...
* **Password Hash Generation:** Password hashes can be generated using `mkpasswd` command-line utility or programmatically using libraries like `passlib` in Python for NETCONF/RESTCONF implementations. **Note:** Modern Debian 12 systems use **Yescrypt** as the default hash function instead of SHA-512, though both are supported for compatibility.
...
5. Hash the password using the same algorithm as `mkpasswd` (e.g., Yescrypt on Debian 12, SHA-512 for compatibility)
