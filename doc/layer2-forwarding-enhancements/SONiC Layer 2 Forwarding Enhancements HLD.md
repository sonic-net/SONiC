...
When a FDB flush request is received, Orchagent internal DB will be traversed and FDB entries corresponding to flush request will be delete from Orchagent data structure and STATE_DB. A delete request to ASIC_DB will also be invoked through sai_redis API.
...
When a port is removed from VLAN, portsorch will call fdborch API to trigger FDB flush. Both static and dynamic FDB will be flushed. Also, the static FDB will be added to the temporary DB in orchagent, so that it can programmed back when port is added back to VLAN.
...
OTgzNzAwLC0yNTk4ODE4NDQsLTE2MjM3Mzc3MTgsODM5NTUzMD
YsMjAwMTczMjI3OCw3MTk1NTE5MDAsLTYxMDMyNDM5NCwxNTMw
NTAwNjA4LDM1ODc4NzYxNyw0NzcyMjcyNjYsLTE5NjAzMjUxND
ksLTE0MDk1MzQ2MjQsLTIwODYyNjIyMzksLTIwOTQzNTk5OTdd
fQ==
-->
