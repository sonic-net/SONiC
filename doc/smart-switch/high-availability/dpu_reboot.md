...
1. SDN controller needs to monitor NPU `STATE_DB` entries below: 
    ...
    1. `DPU_RESET_INFO|DPU<dpu_index>: {'reset_status': 'true|false', 'last_reset_status_update': timestamp}`  
        ...
        Controller wil update the status to `false` when it finishes the service provision and HA programing after DPU is up.        
...
sequenceDiagram
    ...
    hamgrd->>DPU_APPL_DB: hamgrd remove passive BFD sessions
    SDN Controller->>hamgrd: 5. Delete HA_SCOPE_CONFIG and HA_SET_CONFIG
    hamgrd->>DPU_APPL_DB: 6. Delete HA_SET, HA_SCOPE
    hamgrd->>NPU STATE_DB: 7. Delete HA_SCOPE_STATE_TABLE, delete VNet routes.
    DPU->>DPU: 8. DPU boots up 
    DPU->>DPU_APPL_DB: 9. Cleanup Database
    DPU->>DPU_STATE_DB: 10. Cleanup Database
    pmon->>NPU CHASSIS_STATE_DB: 11. Update DPU state to Up
    hamgrd->>DPU_APPL_DB: 12. Create BFD passive sessions. 
    hamgrd->>NPU STATE_DB: DPU_RESET_INFO|DPU0: {"dpu_ready": "true"}

    NPU STATE_DB->>SDN Controller: 13. DPU_RESET_INFO|DPU0: {"dpu_ready": "true"} && CHASSIS_MODULE_TABLE|DPU0 {'admin_status':'up'} &&CHASSIS_MODULE_TABLE|DPU0 {'oper_status':'up'} 
