...
- midplane-network.service needs to create a midplane bridge with the corresponding configuration in the Linux Kernel.
- interface-config.service shall be extended to add the midplane PCIe interfaces to the midplane bridge.
- DHCP server container should be included into the switch image. DHCP server feature should be enabled by default

The midplane bridge configuration shall includes two stages: 1. Create the Linux Kernel bridge and assign a fixed IP address to it in the midplane-network.service 2. Add the PCIe interfaces to the midplane bridge in the interface-config.service. The second stage of midplane bridge configuration and DHCP server configuration shall be generated by sonic-cfggen utility. No user involvement is required.
...
![image](./smart-switch-ip-assignment-midplane-bridge-configuration-flow.svg)

1. Midplane-network.service renders interfaces configuration file /etc/network/interfaces.d/midplane that includes configuration for midplane bridge according to the hwsku.json.
...
2. Reload networking service consumes the interface configuration and configures the midplane bridge with empty ports.
3. After initializing the database, Interface-config service when started consumes configuration from the config DB.
4. Reload networking service consumes the interface configuration and add dpu ports to tidplane bridge.
...
- (1) sonic-cfggen based on hwsku.json renders midplane network and DHCP server configuration.
...
1. Midplane-network.service renders `eth0` interface configuration file /etc/network/interfaces.d/midplane according to the hwsku.json.
