...
- midplane-network.service needs to create a midplane bridge with the corresponding configuration in the Linux Kernel and add the midplane PCIe interfaces to the midplane bridge.
- DHCP server container should be included into the switch image. DHCP server feature should be enabled by default

The midplane network configuration shall includes two stages in the NPU side:

- Stage 1 (midplane-network.service)
  1. Create the Linux Kernel bridge
  2. Assign a fixed IP address to it
  3. Attach the PCIe interfaces to the midplane bridge

- Stage 2 (dhcp-server.service)
  1. DHCP server configuration shall be generated by sonic-cfggen utility. No user involvement is required.
...
2. Reload networking service consumes the interface configuration and configures the midplane bridge
...
2. Add new tests to cover the handling of DPUS tables in sonic-cfggen utility.
3. Add new tests to cover DHCP server configuration generation for the Smart Switch in sonic-cfggen utility.
4. test midplane-network.service to verify midplane bridge configuration generation.
