...
Simplified Switch System Integration Program (a.k.a S3IP) is a sub-project of the Open Data Center Committee (ODCC). It aims to simplify the integration of open networks and promote the development of open source ecology through the specification and open source of software and hardware.

Different platforms have different peripheral hardware, such as PSUs, fan modules, environment sensors, etc. It is a challenge to manage these hardware with a unified interface.

S3IP sysfs is a Linux sysfs-like virtual file system that defines interfaces to access peripheral hardware. The purpose of this specification to construct a hardware abstraction layer to shield the differences of hardware platforms and ensure the portability of hardware monitoring and management applications on other hardware platforms.

The S3IP SYSFS specifiction is not intended as a replacement for the new platform API, it is a useful addition, they do not conflict, and can work together.

In the process of co-development, White-box users often customize hardware (such as CPLD) to improve device availability. Device vendors need to write drivers to export sysfs nodes to access the hardware, and then adapt to higher-level management software through platform API. As device vendors adapt to different hardware platforms, they face the following challenges:

1. For customized hardware, the sysfs path exported by the hardware vendor does not have specifications to rely on. Therefore, the sysfs node is disorganized.

2. The same type of device has different sysfs paths on different hardware platform, so it is impossible to reuse the platform API, and the workload of repeated adaptation is large.

3. Test cases to test sysfs cannot be accumulated.

4. Unorganized sysfs makes it difficult to debug.

The S3IP sysfs specification can solve these problems. Device management application, debugging tools, and test cases can be reused after the manufacturer adapt to the specification.
...
The S3IP sysfs specification represented as an organized sysfs directory structure on white-box devices. Device management software and debugging tools need to access hardware through this interface.

The S3IP sysfs specification is adhered to by both vendors and users. Device vendors focus on the specification implentation, while users verify the usability of the device against this specification.
...
S3IP sysfs framework contains two parts。
 ...
 - S3ip_sysfs_framework: S3IP framework kernel module.
...
![get_flow](/images/s3ip_sysfs/s3ip_sysfs_get_info_flow.svg)
 ...
 - Two device platforms that conform to the S3IP sysfs specifiction will be submitted to the buildimage repository
 - S3ip sysfs framework will be submitted to the sonic-platform-common repository 

##### 3.2.1 s3ip sysfs kernel module

The specification is not only a guidline, it also contains detail informaton. In order to make it easy to meet those detail requirement, the kernel module is designed. 
...
Sample code for the watchdog registration interface, The s3ip_sysfs.ko implements this code
...
Sample code exposes watchdog register to sysfs using s3ip_sysfs.ko, This code should be implemented by the vendor
...
```
### 3.3 Port guide
1. git clone sonic-platform-common to get S3IP framework
2. Verify the availability of S3IP framework on the host computer
...
3. Porting S3IP sysfs framework to platform project
 - Create the S3IP sysfs service configuration file, which controls the /sys_switch directory mapping. This file should be installed on the device path /etc/s3ip/s3ip_sysfs_conf.json
     - The configuration file can be created by modifying the /s3ip/s3ip_sysfs_service/s3ip_sysfs_conf.json file in the source code.
 - Implement the platform hardware driver, and export parameters through s3ip_sysfs.ko
     - s3ip_sysfs. ko is generated by compiling the source code of/s3ip/s3ip_sysfs_frame
 - Install the s3ip-sysfs.service on the device
    - The service file is /s3ip/scripts/ s3IP-sysfs. service, which does not need to be modified. It needs to be installed in /etc/systemd/system/s3ip-sysfs.service on the device.
    - When the platform package is installed, vender should enable s3ip-sysfs.service and start this service automatically when device reboot
    - After the platform package is uninstalled, vender should disable the s3ip-sysfs.service and remove the service

4. Self-test after integration
   - Ensure that the /sys_switch directory conforms to the S3IP sysfs specification and reads and writes data properly
