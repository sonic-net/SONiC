...
- **DSE Discovery Cadence**: Policy values are supplied by the vendor handle. The included platform reference policy performs two bootstrap scans five seconds apart, then three complete monitor/sample/rescan warmup cycles. Unchanged results enter a 300-second stable rescan interval; an inventory change returns the template to warmup. Stable backoff affects discovery only: `get_value()` and a DSE `get_comparator()` still execute at every event sample.
...
| `sonic-host-services` | `dldd/validation.py`, `preflight.py`, `models.py`, `dse.py`, `platform.py` | Bounded JSON/YAML parsing, shared CLI/daemon activation preflight, file/rule isolation, immutable domain objects, typed DSE direct-result/runtime-handle contracts and resolution, platform compatibility, and side-effect-free materialization |
...
| `sonic-host-services` | `dldd/watcher.py`, `lifecycle.py`, `filesystem.py`, `reset.py`, `cli.py`, `scripts/dldd*`, `data/debian/*dldd*` | Stable inbox detection, generation promotion/fallback, shared atomic JSON/file operations, operator runtime-state cleanup, validation CLI, systemd service, watcher service, and timer |
...
| `platform-cisco-8000` | `sonic-platform-modules-cisco/cisco-8000/classes/sonic_platform/dldd.py`, `device/x86_64-8102_28fh_dpu_o-r0/dld_dse.yaml`, `sonic-platform-modules-cisco/cisco-8000/tests/test_dldd.py` | Non-normative vendor-owned platform reference: transport-neutral selector capabilities, private resolver/configuration mapping, live STATE_DB expansion/value/comparison hooks, discovery policy, and unit tests |
