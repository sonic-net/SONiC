...
|                 |                | negative_signal_count | event count to confirm a negative state transition, i.e. active to unknown.                                                                                                                                      |
...
|                    |              |                   state                    | "healthy, unhealthy, uninitialized"; Written by linkmgrd to reflect current combined mux state and link prober state. "uninitialized" indicates that SM for this port did not complete initialization and is considered unhealthy for auto failover. |
