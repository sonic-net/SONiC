...
- [9. Configuration and Management](#9-configuration-and-management)
  ...
  - [9.2 DB Tables](#92-db-tables)
    ...
    - [9.2.6 Segment Lists](#926-segment-lists)
  - [9.3 CLI](#93-cli)
...
- Dynamic candidate-paths are not supported
- SR-TE segment types other than `ipv6-address` (Type B) — other NAI/MPLS
  segment types are reserved in YANG but not implemented in the backend.
...
2. **bgpcfgd** (`SRv6Mgr`, `Srv6GlobalMgr`) and/or **frrcfgd** subscribe to
   these CONFIG_DB tables and render FRR `segment-routing srv6` /
   `segment-routing traffic-eng` CLI, which they push to FRR via `vtysh`.
...
- Only `explicit` SR-TE candidate paths and `ipv6-address` (Type B) segments are
  supported; dynamic/PCE paths and other segment types are not.
