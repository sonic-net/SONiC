...
We extend the STATIC_ROUTE table by adding a `segment` field to enable static routes to be steered over SRv6 SID lists:
...
+ segment = string               ; SRv6 SID list for traffic steering
```
...
We extend **StaticRouteManager** to process the new `segment` field, parsing it and propagating it to FRR.
...
```
ip route 10.0.0.0/24 Ethernet0 segments fcbb:bbbb:2:3:fedd::
ipv6 route 2001:db8:10::/64 Ethernet4 segments fcbb:bbbb:4:5:6:fedd::
```
...
The sonic-static-route YANG model is extended by adding the `segment` leaf to the static route list. The simplified model is shown below:
...
| :------ | :----- |
| add IPv4 static route with segment in CONFIG_DB | verify the static route config entry with SID list is created in FRR config |
| delete IPv4 static route with segment in CONFIG_DB | verify the static route config entry with SID list is removed from FRR config |
| add IPv6 static route with segment in CONFIG_DB | verify the static route config entry with SID list is created in FRR config |
| delete IPv6 static route with segment in CONFIG_DB | verify the static route config entry with SID list is removed from FRR config |
...
| :------ | :----- |
| IPv4 with segment (config) | Configure IPv4 static route with segment and verify SID list and route entry in APPL_DB |
| IPv6 with segment (config) | Configure IPv6 static route with segment and verify SID list and route entry in APPL_DB |
| IPv4 with segment (dataplane) | Configure IPv4 static route with segment and verify dataplane steering |
| IPv6 with segment (dataplane) | Configure IPv6 static route with segment and verify dataplane steering |
