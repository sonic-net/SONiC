...
`upstream-artifacts.yaml` in each repo is used to declare which artifacts/pipelines need to be downloaded for the repo's build environment. This is also how the upstream/downstream relationship is declared. If an artifact listed in this file provides its own `build-env/` dependency files, those dependencies are inherited and that artifact's `upstream-artifacts.yaml` is recursively parsed. Because a branch-versioned `buildenv_setup` can read bundles produced by other branches/commits during this cascade, the schema follows two rules. It is **additive-only** — fields are only ever added, never removed, renamed, or repurposed — so a newer tool can always read older data (validated by the pre-merge gate, §6.8, which runs the PR's tool against the real, already-published upstream artifacts). And the tool **fails loud on unknown fields** rather than silently ignoring them: in this domain an unknown field may encode a *required* setup step, so silently dropping it would produce a subtly-broken environment (a hard-to-debug failure). Failing loud instead surfaces any tool/data version skew — and plain typos — at the introducing PR (via each consumer's own CI), and enforces the correct ordering: tool support for a field must land before any `build-env/` uses it.
...
| 6 | **Cross-branch schema skew.** Because `buildenv_setup` is branch-versioned but its cascade reads `build-env/` bundles produced by other branches/commits, a schema change could make a tool encounter a field it doesn't understand (risking a silently-dropped required step) or fail to parse a bundle. | Two complementary rules (§6.3): the schema is **additive-only** (fields are only added, never removed/renamed/repurposed) so a newer tool always reads older data — validated by the pre-merge gate running the PR's tool against real published artifacts; and the tool **fails loud on unknown fields** rather than ignoring them, so any tool/data skew (or typo) fails at the introducing PR in that consumer's own CI instead of silently producing a broken environment. This enforces that tool support for a field lands before any `build-env/` uses it. |
