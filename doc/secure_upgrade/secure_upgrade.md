...
* 1. [Table of Content](#TableofContent)
	...
	* 1.7. [High-Level Design](#High-LevelDesign)
		* 1.7.1. [This feature has 2 flows to be supported:](#Thisfeaturehas2flowstobesupported:)
		* 1.7.2. [Signing process](#Signingprocess)
		* 1.7.3. [Verification process](#Verificationprocess)
	* 1.8. [SAI API](#SAIAPI)
...
Secure installation and upgrade of OS is setting a requirement to ensure image was not modified since it was built by the vendor. This is a basic requirement as part of general OS security. In order to help the credibility of the OS image, secure upgrade mechanism guarantees image was not modified by any unauthorized entity since its original creation.

###  1.5. <a name='Requirements'></a>Requirements

We want to enable secure upgrade of SONIC. This will include secure upgrade of SONIC while installing SONIC via ONIE or SONIC. The feature requires a signing process which requires a signing private key and certificate and a verification process which requires a public key certificate for validation. Both processes are required to be supported from various scenarios such as development and release.
I order to do secure installation of SONIC from ONIE, it has to include the flowing secure upgrade verification commit TODO (on pull request - waiting for it to be created)
...
We want to add the flags SONIC_SU_MODE_ENABLED and SONIC_SU_PROD_ENABLED to allow several modes of secure upgrade- when SONIC_SU_MODE_ENABLED == n no change will be made to current build process. In case of SONIC_SU_PROD_ENABLED == y SONIC_SU_PROD_ENABLED will determine if we sign with development script (equals 'n') or production script (equals 'y').
![build_options](build_options.png)
- After image creation, during onie-mk-demo.sh script, we create a new output file which includes a prefix sharch.sh script and the image. We calculate sha1 and original image size and update them in the prefix for verification during image install.
- We will call a dedicated script to create an image signature:
We will allow the use of 2 script types in this flow -

	a.  development script - su_sign_image_dev.sh, which will use simple cms openSSL signing (enable with flag set to SONIC_SU_MODE_DEV) with development keys.

	b.  production script - su_sign_image_prod.sh (enable with flag set to SONIC_SU_MODE_PROD) that each vendor will be able to provide for himself. 

![signing_options](signing_options.png)

####  1.7.3. <a name='Verificationprocess'></a>Verification process
- SONIC install – changes will be mainly done in sonic-installer main.py where bootloader methods are being called and will only affect 'sonic-installer install' command (other sonic-installer commands will not be effected). Installation process is being triggered by install.sh script, which is different for every platform.
...
- A signed image will go through a verification process before installation, as main.py will call dedicated bash script to split the signature from the image and check it with public key certificate.

Corresponding to the image build flow we have the following split:

a. on development we will use development script - su_verify_image_sign_dev.sh (enable with flag SONIC_SU_DEV) which will verify using openSSL cms. Certificate will be fetched from BIOS using efi tools.


b. on production we will use su_verify_image_sign_prod.sh (enable with flag SONIC_SU_PROD) that vendors can override as well.
...
Secure installation of SONIC from ONIE requires specific verification code, which can be picked from the following ONIE pull request TODO
...
In this flow signature is not verified as no verification code is available at this stage. If we assume secure upgrade SONIC image is credible, then upgraded OS will support secure upgrade flow.
...
This flow will only be enabled if secure boot BIOS flag is disabled, as we assume user with access to BIOS should have the ability to downgrade OS version.
...
We can use verification script su_verify_image_sign_dev.sh as part of standalone test cases, without the need to run the whole installation process. 

Note - The script have the ability to support use public key that was provided by the user and not only from BIOS. 
...
- Bad flow:
	...
	-  Image with wrong sha1 in sharch
	-  Image with modified signature
	-  Image signed with one key and verified with different key 

####  1.13.2. <a name='SystemTestcases'></a>System Test cases
- Good flow

	- The basic test will include building signed an image and installing it after verification.
- Bad flow
