...
| **CBFC-02** | **Receiver Credit Handling** | Enable CBFC Receiver on DUT Rx port | Inject N frames with varying frame sizes including, but not limited to - 64+x,128+x,256+x,512+x,1518+x,2048+x,4096+x,9216+x bytes where x is 0, +1 or -1 (except for 64 bytes +1 only and for 9216 bytes -1 only) | VC counters `RECEIVER_CREDITS_CONSUMED`, `RECEIVER_CREDITS_FREED` on DUT Rx port matches theoretical formula |
