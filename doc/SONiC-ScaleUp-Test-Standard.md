...
- [SONiC Scale-Up Test Standard](#sonic-scale-up-test-standard)
...
- **Tester:** Must support LLR/CBFC traffic models, sub-microsecond timestamps, CRC error injection, link level impairment.
...
| Case ID | Functional Goal (UEC) | SAI Configuration (API) | Traffic Injection (Data Plane) | Verification Criteria (Stats/Trace) |
|:---|:---|:---|:---|:---|
| **CBFC-01** | **Sender Credit Handling** | Enable CBFC Sender on DUT Tx port| Inject N frames with varying frame sizes including, but not limited to - 64+x,128+x,256+x,512+x,1518+x,2048+x,4096+x,9216+x bytes where x is 0, +1 or -1 (except for 64 bytes +1 only and for 9216 bytes -1 only) | VC counters `SAI_VIRTUAL_CHANNEL_STAT_SENDER_CREDITS_USED`, `SAI_VIRTUAL_CHANNEL_STAT_SENDER_CREDITS_CONSUMED`, `SAI_VIRTUAL_CHANNEL_STAT_SENDER_CREDITS_FREED` on DUT Tx port matches theoretical formula |
| **CBFC-02** | **Receiver Credit Handling** | Enable CBFC Receiver on DUT Rx port | Inject N frames with varying frame sizes including, but not limited to - 64+x,128+x,256+x,512+x,1518+x,2048+x,4096+x,9216+x bytes where x is 0, +1 or -1 (except for 64 bytes +1 only and for 9216 bytes -1 only) | VC counters `SAI_VIRTUAL_CHANNEL_STAT_RECEIVER_CREDITS_CONSUMED`, `SAI_VIRTUAL_CHANNEL_STAT_RECEIVER_CREDITS_FREED` on DUT Rx port matches theoretical formula |
| **CBFC-03** | **Credit Loss/Recovery** | Enable CBFC | 1. Stop sending CC_Update message and then inject 100 corrupted packets <br>2. Resume CC_Update messages | Check VC counters `SAI_VIRTUAL_CHANNEL_STAT_RECEIVER_CREDITS_CONSUMED` and `SAI_VIRTUAL_CHANNEL_STAT_RECEIVER_CREDITS_FREED` on DUT Rx port <br>1. After step 1, there is no change in these counters. <br>2. After step 2, these counters are updated and matches S_VC_CC and S_VC_CF of Sender test port. Additionally S_P_CU = 0 on Sender test port. |
...
| **CBFC-08** | **Per-VC Credit** | Configure 4 lossless VCs in per VC credit mode | 1. Inject traffic on all VCs. <br>2. Create backpressure on individual VCs. | Backpressure on one VC does not affect traffic flow at the other VCs |
| **CBFC-09** | **Per-Port Credit** | Configure 2 lossless VCs in per port credit mode, where all VCs share the port credit on both Tx and Rx side | 1. Inject traffic on both VCs. <br>2. Create backpressure on individual VCs. | 1. Traffic flows at VCs are not affected when no backpressure <br>2. Depending upon implementation, backpressure on VC1 might or might not affect traffic flow at VC2  |
...
**Action Item:** Please confirm with the SAI vendor if the above attributes and counters are supported or planned for the next SDK release.

**Note:** CBFC per-VC credit statistics are already modeled by the existing SAI interface. The queue-level `SAI_QUEUE_STAT_CBFC_XXX` counters listed in previous revisions are replaced by the following per-VC counters from `sai_virtual_channel_stat_t`:

| Existing SAI VC Counter | Description | Usage |
| :--- | :--- | :--- |
| **SAI_VIRTUAL_CHANNEL_STAT_SENDER_CREDITS_USED** | Get sender credits used | CBFC-01 |
| **SAI_VIRTUAL_CHANNEL_STAT_SENDER_CREDITS_CONSUMED** | Get sender credits consumed | CBFC-01 |
| **SAI_VIRTUAL_CHANNEL_STAT_SENDER_CREDITS_FREED** | Get sender credits freed | CBFC-01 |
| **SAI_VIRTUAL_CHANNEL_STAT_SENDER_CREDITS_USED_WATERMARK** | Get sender credits used watermark | CBFC-01 |
| **SAI_VIRTUAL_CHANNEL_STAT_RECEIVER_CREDITS_CONSUMED** | Get receiver credits consumed | CBFC-02/03 |
| **SAI_VIRTUAL_CHANNEL_STAT_RECEIVER_CREDITS_FREED** | Get receiver credits freed | CBFC-02/03 |
