...
The Stats Manager operates as a separate thread within the DHCPv4 relay process and periodically updates per-VLAN relay statistics in the State DB. Additionally, an optional CLI or signal handler allows for on-demand updates to the State DB.
