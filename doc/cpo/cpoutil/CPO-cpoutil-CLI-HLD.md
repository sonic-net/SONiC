...
- [HLD: cpoutil – SONiC CLI Utility for CPO (Co-Packaged Optics)](#hld-cpoutil--sonic-cli-utility-for-cpo-co-packaged-optics)
  ...
  - [10.Memory Consumption \& CPU Usage](#10memory-consumption--cpu-usage)
...
| 1.2 | 2026-09-14 | KroosMicas | Update scope; add descriptions for cpo.json, CPU usage.                    |
| 1.3 | 2026-09-24 | KroosMicas | Update interface dom note                                                  |
...
4. This design only covers the `cpoutil` command; APIs are out of scope for this document.
...
2. Mapping information loaded from static platform file `cpo.json` at command startup. Please refer to the current `cpo.json` schema at : [github.com/KroosMicas/SONiC/blob/4768d007cc353e591a48f152ff0e1f00f22f0c3a/doc/cpo_bailly/Bailly-CPO-support-in-SONiC.md#621-cpojson](https://github.com/KroosMicas/SONiC/blob/4768d007cc353e591a48f152ff0e1f00f22f0c3a/doc/cpo_bailly/Bailly-CPO-support-in-SONiC.md#621-cpojson) or, or the source file: [github.com/sonic-net/sonic-buildimage/blob/master/device/micas/x86_64-micas_m2-w6940-128x1-fr4-r0/cpo.json](https://github.com/sonic-net/sonic-buildimage/blob/master/device/micas/x86_64-micas_m2-w6940-128x1-fr4-r0/cpo.json) . Or refer to the future `cpo.json` schema at: [github.com/sonic-net/SONiC/blob/master/doc/platform/port_mapping_for_cpo.md](https://github.com/sonic-net/SONiC/blob/master/doc/platform/port_mapping_for_cpo.md)
...
> Note: `cpoutil` reuses the existing sfputil key-map scheme and adds  vendor-specific key maps. Reference:[github.com/sonic-net/SONiC/pull/2317](https://github.com/sonic-net/SONiC/pull/2317)
...
## 10.Memory Consumption & CPU Usage
...
4. Baseline memory footprint: < 50MB for normal execution.

**CPU Usage**

1. `cpoutil` is a short‑lived on‑demand CLI utility, no background polling or infinite loop; CPU consumption occurs only during command execution and returns to idle after process exits.
2. Normal `show/config/read‑eeprom/write‑eeprom` commands: low CPU utilization; main cost comes from JSON parsing/formatting and HAL register invocation; CPU burst duration is milliseconds level.
3. Bulk query commands (dump all OE/ELS instances): moderate short‑term CPU spike, caused by iterating hardware instances, collecting status data and formatting table/json output; spike is transient and disappears once command finishes.
4. Under high‑frequency automation script invocation scenario: user is responsible for controlling invocation interval; `cpoutil` itself does not implement built‑in rate‑limiting.
