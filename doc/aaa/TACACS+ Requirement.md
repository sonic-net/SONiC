...
- [1 Functional Requirements](#1-functional-requirement)
  ...
  * [1.6 Multiple TACAS server](#16-multiple-tacas-server)
...
- Only command in whitelist  visible to user:
	- All privilege level share same whitelist to simplify design.
...
- Failover:
	- Use syslog as backup when remote TACACS not avaliable.
...
## 1.6 Multiple TACACS server
 ...
 - When all server not available, use native failover solution.
...
```
    show config tacacs restrict command whitelist
    Config tacacs restrict command whitelist remove <command>
    Config tacacs restrict command whitelist add <command>
```
