...
### NEXT_HOP_GROUP_TABLE
    ...
    ;Status: Mandatory
    key           = NEXT_HOP_GROUP_TABLE:string ; arbitrary identifier for the next hop group
    nexthop       = *prefix,           ; IP addresses separated “,” (empty indicates no gateway)
    ifname        = *PORT_TABLE.key,   ; zero or more separated by “,” (zero indicates no interface)
```
...
Existing applications will not be affected by this enhancement, so no warm upgrade support is needed.
