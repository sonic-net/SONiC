...
- [6. Architecture](#6-architecture)
  ...
  - [6.5 Consumer Call-Site Classification](#65-consumer-call-site-classification)
...
- No logic change required from any next-hop *consumer* orchagent module in the common case -- the existing check-before-use pattern already present in those modules transparently gains the new behavior. The call itself is renamed `hasNextHop()` -> `ensureNextHop()`, and the minority of call sites that only ask whether the next hop exists move to `hasSyncdNextHop()` instead ([Section 6.3](#63-key-apis), [Section 6.5](#65-consumer-call-site-classification)).
...
`hasNextHop()` (since renamed -- see [Section 6.3](#63-key-apis)) was a read-only existence check (`m_syncdNextHops.find(...)`). Every consumer's "does this next hop exist" check trivially succeeded because creation always happened up front.
...
With the gate enabled, the SAI next hop is created either eagerly (unchanged, for the cases that still need it immediately) or lazily, on first reference, via the `ensureNextHop()` call that every consumer already makes, structurally unchanged, before using a next hop.

The consumers affected are `RouteOrch`, `NhgOrch`, `MplsRouteOrch`, `Srv6Orch`, `VNetOrch`, `AclOrch`, `FgNhgOrch`, and `MuxOrch`; [Section 6.5](#65-consumer-call-site-classification) covers which of their call sites become `ensureNextHop()` and which become `hasSyncdNextHop()`.

<a id="63-key-apis"></a>
### 6.3 Key APIs
| Function | Kind | Purpose |
|---|---|---|
| `ensureNextHop(key)` | public, can create | "Ensure this next hop is usable, creating it now if needed." Lazily creates the SAI object on demand if the neighbor is resolved but the object doesn't exist yet. **This is the trigger point for on-demand creation.** Called by consumers that are about to *use* a next hop (RouteOrch, NhgOrch, MplsRouteOrch, Srv6Orch, VNetOrch, AclOrch, FgNhgOrch, MuxOrch); [Section 6.5](#65-consumer-call-site-classification) says which sites qualify. Its existence check is `hasSyncdNextHop()`, so a `MuxOrch` tunnel override satisfies it -- right for callers that go on to call `getNextHopId()`, wrong for callers that need the *local* object ([Section 7.2](#72-muxorch)). |
| `ensureLocalNextHop(key)` | public, can create | `ensureNextHop()` with the existence check narrowed to `m_syncdNextHops` alone, skipping `MuxOrch`. Used only by the mux sites that follow it with `getLocalNextHopId()`, where a tunnel override must not stand in for the neighbor's own next hop ([Section 7.2](#72-muxorch)). |
| `hasSyncdNextHop(key)` | public, read-only | "Does the SAI object exist right now?" Never creates. Used for every internal check inside `NeighOrch` (asserts, removal guards, refcount accessors) where creating would be wrong (e.g. mid-teardown), and by the consumer sites classified as read-only checks in [Section 6.5](#65-consumer-call-site-classification). Public for those consumers and for the unit tests, which call it directly to assert on the read-only/creating split ([Section 13.1](#131-test-plan)). |
| `isNeighborResolved(key)` | public, read-only | "Is the underlying ARP/ND entry resolved?" Decoupled from next-hop existence -- a read-only query against the neighbor table (`hw_configured` flag), independent of whether the SAI next hop has been created. |
...
**Naming note:** the pre-existing API was `hasNextHop()`, a read-only check (see [Section 6.1](#61-before)). Letting it create next hops without renaming it would leave a `has*`-named method that can silently allocate a hardware resource -- a maintenance trap for any future caller that assumes `has*` means "read-only." The rename touches every consumer file that calls it (`routeorch.cpp`, `nhgorch.cpp`, `fgnhgorch.cpp`, `muxorch.cpp`, `vnetorch.cpp`, `srv6orch.cpp`, `mplsrouteorch.cpp`, `aclorch.cpp`), a wider diff than the behavioral change alone required, but judged worth it for API contract clarity over minimizing file count.

**Read-only vs. creating calls (`neighorch.cpp`):** the design only holds if every internal check that must not create anything calls `hasSyncdNextHop()` rather than `ensureNextHop()`; `isNeighborResolved()` uses a separate query altogether. Every such call site in `NeighOrch`, production and test, is listed below so the split can be checked rather than taken on trust:

- **`ensureNextHop()` itself:** the fast path -- `if (hasSyncdNextHop(nexthop)) return true;` returns immediately without creating anything if the object already exists, before ever considering creation. `ensureLocalNextHop()` is the same, except its fast path tests `m_syncdNextHops` alone and does not count a `MuxOrch` tunnel override as existing ([Section 7.2](#72-muxorch)).
...
- **`getNextHopId()`:** `assert(hasSyncdNextHop(nexthop))` before returning the SAI object id -- a read-only accessor that must never itself trigger creation.
...
- **`removeNeighbor()`:** the one non-`assert`, decision-making use, and the one internal site that does **not** go through `hasSyncdNextHop()` at all: it looks up `m_syncdNextHops.find(nexthop)` directly to decide both whether to attempt a SAI `remove_next_hop()` call and where to read the object id from, specifically to avoid `hasSyncdNextHop()` also counting a `MuxOrch` tunnel override as existing (see [Section 7.2](#72-muxorch)) and `operator[]`'s silent-insert behavior, either of which could otherwise treat a deferred, never-created next hop as existing and drive removal off a zeroed entry that was never real.
...
Inside `neighorch.cpp` the split is complete as listed above; consumers outside it are covered in [Section 6.5](#65-consumer-call-site-classification).
...
<a id="65-consumer-call-site-classification"></a>
### 6.5 Consumer Call-Site Classification
`hasNextHop()` is used two ways today, and only one of them may become `ensureNextHop()`. Each call site outside `neighorch.cpp` is classified before conversion:

1. **Check-before-use -> `ensureNextHop()`.** The caller is about to use the next hop and needs it to exist, and reads the id right after. Examples: `RouteOrch::addRoute()`, `NextHopGroupMember::getNhId()`, `FgNhgOrch` activating a member. This is the common case, and converting it is what makes on-demand creation work.
2. **Read-only check -> `hasSyncdNextHop()`.** The caller only wants to know whether the next hop exists, usually to decide something on a teardown path. Creating one here would be wrong.

Two sites in the second group show why a blanket rename would break things:

- **`NextHopGroupMember::~NextHopGroupMember()` (`nhgorch.cpp`)** checks `hasNextHop(m_key) && getNextHopRefCount(m_key) == 0` before removing. If that call created, the destructor would create a SAI next hop and immediately delete it.
- **`RouteOrch::removeNextHopGroup()` (`routeorch.cpp`)** uses `!hasNextHop(nhop->first)` to skip a member whose next hop is missing. If the call created, the condition flips and the other branch runs.

Two more sites need a close look during conversion: the MPLS branch of `RouteOrch::addNextHopGroup()` (creating there would force the underlying IP next hop into existence as a side effect of adding a group member) and the post-install check in `RouteOrch::addRoutePost()`.

Mux adds a third case: sites that need the neighbor's **own local** next hop use `ensureLocalNextHop()` -- see [Section 7.2](#72-muxorch).
...
1. **Add side.** `NeighOrch::addNeighbor()` explicitly calls `gFgNhgOrch->validNextHopInNextHopGroup(nhKey)` for every plain neighbor, restoring the "neighbor resolved" notification. `FgNhgOrch::validNextHopInNextHopGroup()` in turn calls `m_neighOrch->ensureNextHop(nexthop)` itself before activating a member, since being configured as an FG ECMP member is itself a legitimate reference that should trigger lazy creation (this is safe: creating the next hop makes `addNextHop()` send its own `validNextHopInNextHopGroup()` notification for the same key, and that repeat notification returns without redoing the work). As a direct consequence, any next hop that FG ECMP actually activates is guaranteed to exist as a SAI object from that point on, since activation is what creates it.
...
<a id="72-muxorch"></a>
### 7.2 MuxOrch
Mux neighbors in the default ("host-route") mode are plain (non `prefix_route`) neighbors and are therefore subject to delayed creation. Several mux code paths (`MuxNbrHandler::update()`/`enable()`, `MuxOrch::updateRoute()`) fetch a next hop's SAI id via the **read-only** `getLocalNextHopId()` lookup, which consults `m_syncdNextHops` only and silently returns `SAI_NULL_OBJECT_ID` for a deferred-but-not-yet-created next hop. Each of these call sites must force on-demand creation before that lookup. (`MuxPrefixBasedNbrHandler`, used for no-host-route mode, is unaffected -- it is always `prefix_route`, hence always eager.)

**`ensureNextHop()` is the wrong call here.** `hasSyncdNextHop()` -- which `ensureNextHop()` uses as its fast path -- also consults `MuxOrch::getNextHopId()`, returning `true` if *either* `m_syncdNextHops` has the key *or* `MuxOrch` reports an active tunnel next hop override for it. Counting the override is correct for ordinary consumers: while a mux neighbor is standby, `getNextHopId()` should return the tunnel next hop, and `RouteOrch` wants exactly that. But it breaks the mux sites above -- with an override present, `ensureNextHop()` reports success having created nothing, and the following `getLocalNextHopId()` still returns `SAI_NULL_OBJECT_ID`, the very failure those sites exist to prevent.

`MuxNbrHandler::enable()` hits this on every standby-to-active transition: it iterates `neighbors_`, whose entries still hold the tunnel id set by the standby path, so the override satisfies the check every time. The `MUX_STATE_ACTIVE` case in `MuxNbrHandler::update()` happens to be safe only because it runs when the key is absent from `neighbors_`, so the override reads null. That is incidental, and not something to rely on.

**Design:** `NeighOrch` adds `ensureLocalNextHop()` ([Section 6.3](#63-key-apis)), which is `ensureNextHop()` with its existence check narrowed to `m_syncdNextHops.find()` alone. The mux sites call it before `getLocalNextHopId()`; every other consumer keeps `ensureNextHop()`, which goes on counting a tunnel override as existing.

Counting the override is also why `removeNeighbor()`'s decision of whether this neighbor's *own* SAI next hop needs tearing down must not rely on `hasSyncdNextHop()`: doing so would treat a mux'd, still-deferred neighbor's next hop as "existing" purely because of the unrelated tunnel override, and then reading `m_syncdNextHops[nexthop]` via `operator[]` for a key genuinely absent from the map would silently insert a zeroed entry, drive `remove_next_hop()` with a null object id off an entry that was never real, and risk a CRM `used` and router-interface-refcount underflow. `removeNeighbor()` therefore checks `m_syncdNextHops` directly (`find()`, never `operator[]`) for this specific decision, which only `NeighOrch`'s own map can answer correctly.

<a id="73-crm-and-reference-counting"></a>
### 7.3 CRM and Reference Counting
- CRM next-hop counters (`CRM_IPV4_NEXTHOP` / `CRM_IPV6_NEXTHOP` / `CRM_MPLS_NEXTHOP`) are incremented/decremented exclusively inside `addNextHop()` / `removeNextHop()` / `removeMplsNextHop()` / `removeOverlayNextHop()`, unchanged. Only the *timing* of creation moves; the one-to-one match between counter and SAI object is untouched, and counts now more accurately reflect real hardware usage than before. The debounced reclaim in [Section 7.6.3](#763-bounding-crm-staleness-debounced-reclaim-of-idle-next-hops) is a new *caller* of `removeNextHop()`, not a new decrement path, so that match still holds.
- `getNextHopRefCount()` / `increaseNextHopRefCount()` / `decreaseNextHopRefCount()` still `assert(hasSyncdNextHop(key))` (read-only check, never creates). Every caller across the tree is audited to confirm it calls `ensureNextHop()` (or `ensureLocalNextHop()`) before touching ref counts; two gaps were found and are fixed by this design (the `FgNhgOrch` and `MuxOrch` items above). This audit looks for sites that must create; [Section 6.5](#65-consumer-call-site-classification) looks for sites that must not. Both are needed.
...
Pending debounced reclaims do not survive the restart; a post-reconciliation sweep re-schedules them for any unreferenced next hop, as described in [Section 7.6.3](#763-bounding-crm-staleness-debounced-reclaim-of-idle-next-hops).
...
Creation of a plain neighbor's SAI next hop may now be deferred, but its *destruction* is never driven directly by `RouteOrch`'s next-hop reference count reaching zero -- `RouteOrch::removeNextHopRoute()` / `decreaseNextHopRefCount()` only ever decrement the in-memory `ref_count` field on a plain next hop; nothing in that path calls `NeighOrch::removeNextHop()` itself. This is what makes the churn-free property below hold. Read in isolation this looks like an unconditional win, but it is also this design's main limitation: taken to its extreme (a next hop referenced once and never again, with its neighbor remaining resolved indefinitely), it would strand that next hop's hardware slot until the neighbor entry itself eventually goes away -- i.e. next-hop occupancy would track *ever-referenced* neighbors (a running total that only grows) rather than *currently*-referenced ones. [Section 7.6.3](#763-bounding-crm-staleness-debounced-reclaim-of-idle-next-hops) is the other half of this trade-off: reference count reaching zero does schedule a reclaim, just a debounced one, so that extreme is bounded to a short grace period instead of being permanent.

Consequently, once a plain next hop has been lazily created by its first route/next-hop-group reference:

- A `SET` (route install) after a `DEL` (route withdraw) for the same prefix, with the neighbor still resolved and the re-reference arriving **within the grace period** ([Section 7.6.3](#763-bounding-crm-staleness-debounced-reclaim-of-idle-next-hops)), calls `ensureNextHop()` again, finds `hasSyncdNextHop() == true`, cancels the pending reclaim, and returns -- no `create_next_hop()`/`remove_next_hop()` SAI call, only `increaseNextHopRefCount()`/`decreaseNextHopRefCount()` bookkeeping.
- **Past the grace period this no longer holds, by design.** The reclaim fires, the next hop is destroyed once, CRM `used` drops, and a later `SET` pays the creation cost again. The grace period is sized so that ordinary flap stays on the churn-free path while a genuinely idle next hop does not hold its slot forever.
- So the one-time creation cost is paid **once, at first reference**, for any churn cycle that finishes within the grace period, rather than once per cycle. Steady-state route flapping costs no more than before this change; only the timing of that one-time cost changes (first route reference instead of neighbor resolution).
...
- As a side effect, CRM `NEXTHOP` used-counters for plain neighbors stay flat across route churn that re-references within the grace period, instead of ticking down/up on every flap, which is a useful property when using those counters as a capacity signal (see [Section 7.6.1](#761-adjacency-resolution-no-longer-reserves-a-next-hop)) -- churn noise does not show up as CRM churn for the common case.

If the churn event also affects the neighbor itself (e.g. an interface flap that ages out and re-resolves the ARP/ND entry, not just a RIB-level route flap), `removeNeighbor()` does tear down the SAI next hop if it existed, so the next reference after re-resolution pays the lazy-creation cost again -- but this exactly mirrors the pre-change cost profile, which always paid this cost on every re-resolution regardless of route churn. No regression there either.

**Churn amplifies the table-full interaction described above.**

The retry-cache merge logic in `Orch::addToSync()` (`orch.cpp`) already handles fast add/withdraw/add churn gracefully at the queueing level: a `DEL` arriving while an earlier `SET` for the same key is parked in the retry cache (e.g. because `ensureNextHop()` previously failed with `SAI_STATUS_TABLE_FULL`/`SAI_STATUS_INSUFFICIENT_RESOURCES`) evicts that pending `SET` outright, and a subsequent re-`SET` is treated as a fresh task. This is beneficial: churn does not cause an ever-growing backlog of retries that cannot succeed against an already-exhausted next-hop table.
...
Before this change this could not happen, because `addNextHop()` ran unconditionally and synchronously inside `addNeighbor()`, so `m_neighborToResolve` was always cleared at the same time as resolution, independent of whether any route ever used the neighbor. Decoupling "next hop created" from "neighbor resolved" (a deliberate and necessary part of this design, see `isNeighborResolved()` in [Section 6.3](#63-key-apis)) reintroduces this as a possible ordering: a route can now request resolution and be withdrawn again before the one event (`addNextHop()`) that used to double as the resolution-request cleanup.
...
Had this not been fixed, the impact would have been bounded and not a hardware resource leak -- no SAI object and no CRM impact, since one was never created -- but leftover bookkeeping state: an internal `std::set` entry (bounded by neighbor table size) plus a stale, never-revisited row per affected neighbor in `APP_NEIGH_RESOLVE_TABLE_NAME`.
...
This is deliberately narrow in scope: it only bounds staleness under ordinary churn, and does not attempt to reactively reclaim capacity under table-full pressure (a different problem, not addressed here).

**Parameters and mechanism.** The grace period is **5 seconds** and the pending-reclaim queue is swept **once per second**, both fixed compile-time constants (`NEXTHOP_RECLAIM_GRACE_PERIOD_SECS`, `NEXTHOP_RECLAIM_SWEEP_INTERVAL_SECS`) with no CONFIG_DB knob. Five seconds covers the flap and reconverge timescales in [Section 7.6.2](#762-behavior-under-frr-route-churn-withdraw--re-inject) while keeping CRM `used` staleness bounded. A `SelectableTimer` owned by `NeighOrch`, registered like any other orchagent timer, drives the sweep. Pending reclaims sit in a queue ordered by due time, alongside a set of the keys still due to be reclaimed. Every entry waits the same grace period, so the queue is already in due-time order and each sweep only has to look at the front. Cancelling a reclaim -- the ordinary churn case -- simply removes the key from that set; the queue entry is left where it is and ignored when the sweep reaches it, so re-referencing a next hop never has to search or rebuild the queue. If the neighbor is removed while a reclaim is still pending, `removeNeighbor()` tears down as usual and the pending entry is dropped the same way.

**Warm restart.** The queue is in-memory and does not survive a warm boot: pending reclaims are discarded, and reference counts are rebuilt by reconciliation ([Section 7.5](#75-warm-restart)). A reclaim is scheduled when a reference count *reaches* zero, so a next hop that was already idle before the restart would never be reclaimed after it -- that event has passed. To close this, `NeighOrch` walks `m_syncdNextHops` once at the end of reconciliation and schedules a reclaim for every plain next hop sitting at zero.
...
Operators should continue to use the existing CRM show commands (`CRM_IPV4_NEXTHOP` / `CRM_IPV6_NEXTHOP` / `CRM_MPLS_NEXTHOP` used/available counters), with `available` remaining the primary, precise signal for approaching next-hop scale limits and `used` serving as a supporting trend rather than a live measure of demand (see [Section 7.6.1](#761-adjacency-resolution-no-longer-reserves-a-next-hop)).
...
- The read-only/creating split is a convention, not something the type system enforces. A future caller that picks `ensureNextHop()` on a teardown or decision path brings back the hazard in [Section 6.5](#65-consumer-call-site-classification). The naming makes that visible in review but cannot prevent it.
...
- Unit tests added in `neighorch_ut.cpp`:
  ...
  - A mux'd neighbor whose local next hop is still deferred (never created) is removed while `MuxOrch` reports an active tunnel next hop override for the same key: `removeNeighbor()` does not treat the local next hop as existing, issues no `remove_next_hop()` call, does not insert an empty `m_syncdNextHops` entry, and does not decrement CRM or the router-interface refcount (see [Section 7.2](#72-muxorch)).
  - A mux'd neighbor whose local next hop is still deferred goes standby-to-active while `MuxOrch` still reports a tunnel override for the key: `ensureLocalNextHop()` creates the local object and `getLocalNextHopId()` returns a valid id instead of `SAI_NULL_OBJECT_ID` (see [Section 7.2](#72-muxorch)).
  - Tearing down an unreferenced `NextHopGroupMember` whose next hop was never created issues no `create_next_hop()` call, confirming the teardown path uses the read-only check (see [Section 6.5](#65-consumer-call-site-classification)).
  - Warm-restart reconciliation ends with a plain next hop at reference count zero: the post-reconciliation sweep schedules a reclaim, which fires once after the grace period (see [Section 7.6.3](#763-bounding-crm-staleness-debounced-reclaim-of-idle-next-hops)).
