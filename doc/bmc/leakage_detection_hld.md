...
```mermaid
flowchart LR
    P[Physical sensor reading] --> PS[Physical per-sensor state]
    ...
    TA --> D
    TA --> L[test_leak = Enabled]
    D --> B{Test leak?}
    L --> B
    B -->|Yes| A[Audit policy outcome only]
    B -->|No| E[bmcctld policy enforcement]

    H[Physical sensor health] --> S[leak_sensor_status]
    TF[Test fault injection] --> S
    TF --> G[test_sensor_fault = Enabled]
    S --> I[System health and CLI]
    G --> I

    PS ~~~ TS
```

Physical and test leaks both use `device_leak_status`. `test_leak=Enabled` identifies a test aggregate and causes `bmcctld` to audit rather than dispatch host control. Physical and test sensor faults both use `leak_sensor_status`; `test_sensor_fault=Enabled` preserves test provenance. Hardware test bits are used only for controlled test validation.
...
The thread records physical leak state and optional test-injection state separately. Test injection follows the normal leak aggregation path and is written to `device_leak_status`; `test_leak=Enabled` on the aggregate record preserves its provenance for `bmcctld`.
...
| Test injection | `device_leak_status` and `test_leak=Enabled` | Audits the outcome that would apply; never dispatches a host-control action |

### Test-fault semantics

When `test_sensor_fault=Enabled`, `thermalctld` uses the normal per-sensor fault-reporting path: it publishes `leak_sensor_status=Fault`, sets `leaking` and `leak_status` to `N/A`, and sets `leak_severity` to `None`, matching the representation of a physical sensor fault. `test_sensor_fault=Enabled` preserves its provenance for CLI and system health. A test-injected fault does not contribute to `device_leak_status` and never causes host control. Clearing the test fault restores reporting from the physical sensor. No aggregate fault-status field is introduced.
...
6. A test-only critical aggregate is written to `device_leak_status` with `test_leak=Enabled`, is audited, and never invokes graceful shutdown, power-off, power-cycle, GNOI, or a platform power-control API.
...
11. Test injection publishes the expected `test_leak_severity` and `leak_source`, and sets the aggregate `test_leak` provenance field.
