...
PDDF provides generic user space python object classes which are derived from 2.0 platfrom APIs defined under:
...
The PDDF generic device object-classes make use of the per platform JSON descriptor files and use the appropriate SysFS attributes to get and set. The final object classes would be derived form these PDDF generic classes and provide users the ability to overwrite/re-define generic get/set implementation under PDDF.
