...
* [5 VOQ Forwarding in Disaggregated Networks](#5-disaggregated-voq-model)
...
# 5 VOQ Forwarding in Disaggregated Networks
...
### 5.3.2 Chassis DB Initialization
...
The eth0 ip address that is associated with the redis_chassis.server is used for reachability to the redis_chassis instance and the local orchagent connects to the local chassis DB hosted on this instance for updating the state information of the local interfaces. The method of updating remote neighbor state infomration to the local chassis DB is out of scope of the document.
