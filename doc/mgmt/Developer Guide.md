...
* [2 Developer Workflow](#2-developer-workflow)
    ...
    * [2.11 gNMI](#211-gnmi)
        * [2.11.1 gNOI from YANG](#2111-gnoi-from-yang)
...
| 1.5  | 06/06/2022 |      Mohammed Faraaz C  | Added details for gNOI from YANG.                            |
...
#### 2.11.1 gNOI from YANG

The gNMI server and the client will be automatically extended to support the RPCs defined in the YANG models (both OpenConfig and SONiC models are supported). Therefore eliminating a need for the developers to extend the gNMI server manually. 

The gNOI calls for YANG RPCs will land in **translib.Action**, therefore providing a common implementation for both gNOI and RESTCONF RPCs.

The gNOI from YANG is facilitated by a newly written pyang plugin for protobuf generation. Refer [YANG RPC to Protobuf](#3110-yang-rpc-to-protobuf) for more information.
...
```
#### 3.1.10 YANG RPC to Protobuf

A new plugin **sonic-telemetry/tools/pyang_plugins/protobuf.py** is written to autogenerate the protobuf messages for YANG RPCs. In addition to the protobuf, the Golang server and the client code to support the generated RPCs will also be autogenerated.

##### 3.1.10.1 Rules

1. It generates proto service definition for each of the YANG module(only models which contain RPCs are considered) in a separate .proto file.
2. It generates proto RPC definition for each of the YANG RPCs under the yang models service as described in step-1.
3. It generates proto messages for each of the YANG nodes for the RPCs generated in step-2.
	
##### 3.1.10.2 Data type mappings for yang to protobuf
...
**Client code:** sonic-telemetry/build/gnoi_yang/client
