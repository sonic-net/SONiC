...
model, wheel packaging, and atomic installation by the gNMI sidecar. It did not
implement Network Device Manager (NDM) policy publication or runtime policy
enforcement. This design does not assume that change is present in a released
or deployed SONiC image.
...
| Golden configuration | The complete desired device configuration generated by NDM. |
| gNMI | gRPC Network Management Interface. |
| gNOI | gRPC Network Operations Interface. |
| gNSI | gRPC Network Security Interface. |
| NDM | Network Device Manager, the producer of the golden device configuration. |
...
- A candidate golden configuration must validate against the active model.
...
| NDM | Generate one complete policy registry and publish it only after compatibility checks pass. |
...
| Deployment system | Order sidecar rollout, NDM publication, upgrade, and rollback. |
...
- NDM and authorized local configuration tools can grant or remove access.
...
The sidecar establishes schema readiness before NDM publishes policy data.
...
sequenceDiagram
    ...
    NDM->>Gate: Submit complete candidate registry
    Gate->>Host: Load active model
    Gate->>Gate: Validate complete candidate
    alt compatible
        Gate-->>NDM: Ready
        NDM->>DB: Publish one configuration transaction
        DB-->>Runtime: Policy change notification
        Runtime->>Runtime: Validate and atomically activate snapshot
    else incompatible
        Gate-->>NDM: Reject candidate
        NDM--xDB: Do not publish
    end
```

The committed `CONFIG_DB` transaction is the registry version 1 publication
boundary. NDM must publish principal, method, and Pathz changes in one atomic
transaction. A change notification is only a reload trigger. After the commit,
each consumer must read all three tables, validate them as one snapshot, and
atomically replace its active snapshot.
...
NDM must compute a stable digest of the complete candidate registry. Each
required runtime consumer must report the digest of its active snapshot through
an operational readiness interface. Runtime activation is complete only after
all required consumers report the expected digest. The digest and readiness
interface are operational state. They are not new `CONFIG_DB` policy fields.
...
streams, and report an unhealthy state. After NDM restores the previous
committed registry, the consumer can reactivate the corresponding validated
last-known-good snapshot. If no last-known-good snapshot exists, it must remain
closed.
...
5. Publish the candidate through NDM.
...
| Later update is invalid | Stop protected traffic and retain the last-known-good snapshot. Do not serve it until repair or rollback. |
...
consumers belong in their respective gRPC server repositories. NDM generation
and deployment orchestration remain separate delivery boundaries.
...
This revision adds no CLICK, KLISH, REST, or operator CLI commands. NDM golden
configuration is the first-phase policy source.
...
2. Define the implementation of the schema revision signal and atomic NDM
   publication handoff.
