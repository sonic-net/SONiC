  ...
  * [3 Design](#3-design)
    ...
    * [3.3 User Interface](#33-user-interface)
      * [3.3.1 Data Models](#331-data-models)
        * [3.3.1.1 Extended OpenConfig Model (In Scope)](#3311-extended-openconfig-model-in-scope)
...
#### 3.3.1.1 Extended OpenConfig Model (In Scope)
This section covers only the extension nodes that appear in the YANG tree shown in this document.

The implementation uses `openconfig-network-instance-ext.yang` (`oc-network-instance-ext`) to represent SONiC-specific BGP and routing behavior without changing the base OpenConfig structure. In this document tree, the following extension areas are in scope:

- BGP global behavior:
  `enable-default-ipv4-unicast-afi`, `max-dynamic-neighbor-prefixes`, and graceful-restart `preserve-fw-state`.
- BGP route-selection and policy controls:
  `network-import-check`, global `ebgp-requires-policy`, and AFI-SAFI import policy extension under global context.
- BGP network advertisement under AFI-SAFI:
  extension container `networks/network/prefix`.
- BGP operational controls:
  extension `logging-options/log-neighbor-state-changes` and extension `timers/hold-time`, `timers/keepalive-interval`.
- Peer-group and neighbor AFI-SAFI behavior:
  extension `soft-reconfiguration-inbound`.
- Aggregate route behavior:
  extension `summary-only` under local-aggregates.

These extension nodes are modeled as OpenConfig-compatible leaves/containers and are mapped to SONiC DB fields via annotation and transformer logic in `sonic-mgmt-common`.
...
|--------------------------------------|-------------------------------|
|   network-instance/config/name       |      CONFIG_DB:VRF key `<name>` (no dedicated field)             |
|   network-instance/state/name        |      APPL_DB:VRF key `<name>` (read path uses APPL_DB)           |
|   network-instance/config/type       |      Derived by transformer from key (no direct DB field)         |
|   network-instance/state/type        |      Derived by transformer from key (no direct DB field)         |
...
|   bgp/global/graceful-restart/config/enabled        |      BGP_GLOBALS/graceful_restart_enable    |
|   bgp/global/graceful-restart/config/restart-time   |      BGP_GLOBALS/gr_restart_time    |
|   bgp/global/graceful-restart/config/stale-routes-time |   BGP_GLOBALS/gr_stale_routes_time |
|   bgp/global/graceful-restart/config/oc-network-instance-ext:preserve-fw-state | BGP_GLOBALS/gr_preserve_fw_state |
|   bgp/global/oc-network-instance-ext:timers/config/oc-network-instance-ext:hold-time | BGP_GLOBALS/holdtime |
|   bgp/global/oc-network-instance-ext:timers/config/oc-network-instance-ext:keepalive-interval | BGP_GLOBALS/keepalive |
|   bgp/global/route-selection-options/config/external-compare-router-id | BGP_GLOBALS/external_compare_router_id |
|   bgp/global/oc-network-instance-ext:logging-options/config/oc-network-instance-ext:log-neighbor-state-changes | BGP_GLOBALS/log_nbr_state_changes |
|   bgp/global/config/oc-network-instance-ext:max-dynamic-neighbor-prefixes | BGP_GLOBALS/max_dynamic_neighbors |
|   bgp/global/config/oc-network-instance-ext:enable-default-ipv4-unicast-afi | BGP_GLOBALS/default_ipv4_unicast |
|   bgp/global/apply-policy/config/oc-network-instance-ext:ebgp-requires-policy | BGP_GLOBALS/ebgp_requires_policy |
|   bgp/global/route-selection-options/config/oc-network-instance-ext:network-import-check | BGP_GLOBALS/network_import_check |

## BGP Neighbor Mapping
|   OpenConfig YANG                              |    SONiC DB Table                    |
|------------------------------------------------|--------------------------------------|
|   bgp/neighbors/neighbor[neighbor-address=*]        |      BGP_NEIGHBOR key (vrf, neighbor-address) |
|   bgp/neighbors/neighbor/config/peer-group          |      BGP_NEIGHBOR/peer_group_name     |
|   bgp/neighbors/neighbor/config/peer-as             |      BGP_NEIGHBOR/asn                 |
|   bgp/neighbors/neighbor/config/description         |      BGP_NEIGHBOR/name                |
|   bgp/neighbors/neighbor/timers/config/hold-time    |      BGP_NEIGHBOR/holdtime            |
|   bgp/neighbors/neighbor/timers/config/keepalive-interval | BGP_NEIGHBOR/keepalive          |
|   bgp/neighbors/neighbor/enable-bfd/config/enabled  |      BGP_NEIGHBOR/bfd                 |

## BGP Peer Group Mapping
|   OpenConfig YANG                                   |    SONiC DB Table                     |
|-----------------------------------------------------|---------------------------------------|
|   bgp/peer-groups/peer-group[peer-group-name=*]     |      BGP_PEER_GROUP key (vrf, peer-group-name) |
...
|   bgp/peer-groups/peer-group/transport/config/local-address    |      BGP_PEER_GROUP/local_addr        |
|   bgp/peer-groups/peer-group/ebgp-multihop/config/enabled      |      BGP_PEER_GROUP/ebgp_multihop     |
|   bgp/peer-groups/peer-group/ebgp-multihop/config/multihop-ttl |      BGP_PEER_GROUP/ebgp_multihop_ttl        |
|   bgp/peer-groups/peer-group/timers/config/connect-retry       |      BGP_PEER_GROUP/conn_retry        |
|   bgp/peer-groups/peer-group/timers/config/hold-time           |      BGP_PEER_GROUP/holdtime          |
|   bgp/peer-groups/peer-group/timers/config/keepalive-interval  |      BGP_PEER_GROUP/keepalive         |
|   bgp/peer-groups/peer-group/enable-bfd/config/enabled         |      BGP_PEER_GROUP/bfd               |
|   bgp/peer-groups/peer-group/graceful-restart/config/enabled   |      BGP_PEER_GROUP/graceful_restart_enable |

## Local Aggregates Mapping
|   OpenConfig YANG                                   |    SONiC DB Table                            |
|-----------------------------------------------------|----------------------------------------------|
|   local-aggregates/aggregate[prefix=*]              |      BGP_GLOBALS_AF_AGGREGATE_ADDR key (vrf,afi-safi, ip_prefix) |
|   local-aggregates/aggregate/config/prefix          |      BGP_GLOBALS_AF_AGGREGATE_ADDR/ip_prefix |
|   local-aggregates/aggregate/config/oc-network-instance-ext:summary-only | BGP_GLOBALS_AF_AGGREGATE_ADDR/summary_only |

## Table Connections Mapping
|   OpenConfig YANG                                   |    SONiC DB Table                            |
|-----------------------------------------------------|----------------------------------------------|
|   table-connections/table-connection[src-protocol,dst-protocol,address-family] | ROUTE_REDISTRIBUTE key   (vrf, src_protocol, dst_protocol, addr_family) |
|   table-connections/table-connection/config/import-policy | ROUTE_REDISTRIBUTE/route_map |
