...
| 0.2 | 02/25/2026  | Anukul Verma          | Adding mapping for NTP server, cpu state, and mount point nodes |
...
- Openconfig-system.yang version 3.3.0 - latest openconfig yang repo version is considered.
...
3. Add support for following System features:
    ...
    * messages
    * alarms
    * mount-points (including inodes-available, inodes-total, inodes-utilized, read-only)
    * memory
    * cpus
    * grpc-servers
    * gnmi-pathz-policies
...
## 3.3 User Interface
### 3.3.1 Data Models
Openconfig-system.yang (revision 3.3.0) and its submodules will be used as user interfacing models.  
We are updating openconfig-system yang version (0.7.0) in sonic with latest available openconfig version (revision 3.3.0), to have all the latest changes from community.  
...
| type | - | - | Type |
| inodes-available | - | STATE_DB | InodeAvailable |
| inodes-total | - | STATE_DB | InodeTotal |
| inodes-utilized | - | STATE_DB | InodeTotal - InodeAvailable (derived; no dedicated field) |
| read-only | - | STATE_DB | ReadOnly |
| **clock** | | | |
...
| state/association-type | sonic-ntp.yang | CONFIG_DB | association_type |
| state/iburst | sonic-ntp.yang | CONFIG_DB | iburst |
| state/network-instance | sonic-ntp.yang | CONFIG_DB | NTP:vrf |
| state/source-address | sonic-ntp.yang | CONFIG_DB | NTP:src_intf |
| state/key-id | sonic-ntp.yang | CONFIG_DB | key |
| state/stratum | - | STATE_DB | stratum |
| state/root-delay | - | STATE_DB | root_delay |
| state/root-dispersion | - | STATE_DB | root_dispersion |
| state/offset | - | STATE_DB | offset |
| state/poll-interval | - | STATE_DB | poll_interval |
| **ssh-server** | | | |
...
3. system/ntp -  
   ...
   vi. GET of server `state` leaves association-type, iburst, network-instance, source-address, and key-id is a replica of config from CONFIG_DB (`NTP_SERVER` / `NTP|global`). Live STATE_DB fields are port, version, stratum, root_delay, root_dispersion, offset, and poll_interval.  
   vii. Deleting the last NTP server also clears `NTP|global` `src_intf` and `vrf`.  
...
5. system/mount-points - `inodes-available`, `inodes-total`, `inodes-utilized`, and `read-only` are base OpenConfig 3.3.0 leaves (STATE_DB `MOUNT_POINTS`). `inodes-utilized` is derived as `InodeTotal - InodeAvailable`.
6. logging & messages severity - EMERGENCY, ALERT & CRITICAL, all these three are translated to SONiC critical level
7. logging facility - Only ALL is supported currently
...
13. Verify that operations supported for gNMI/REST works fine for mount-points nodes (including inodes-available, inodes-total, inodes-utilized, read-only).
