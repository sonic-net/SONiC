...
- [4. Overview](#4-overview)
  - [4.1 Telemetry](#41-telemetry)
    - [4.1.1 IFA2 packet format and encapsulation](#411-ifa2-packet-format-and-encapsulation)
  - [4.2 Monitoring](#42-monitoring)
- [5. Requirements](#5-requirements)
  - [5.1 Phase 1](#51-phase-1)
  - [5.2 Phase 2](#52-phase-2)
  - [5.3 Phase 3](#53-phase-3)
- [6. Module Design](#6-module-design)
  ...
  - [6.2 Configuration and control flow](#62-configuration-and-control-flow)
    - [6.2.1 Inband Telemetry](#621-inband-telemetry)
    - [6.2.2 Drop Monitoring](#622-drop-monitoring)
  - [6.3 SWSS and syncd changes](#63-swss-and-syncd-changes)
...
|v0.2|2025-11-17|Prashanth Krishnamurthy|Added design for MOD

## 2. Scope
This document describes the high level design of Telemetry and Monitoring (TAM) in SONiC. It covers the dataplane implementation of the TAM producer, and the wire formats for exporting the telemetry data.
...
## 4. Overview
TAM provides a framework to observe and report network events and telemetry by exporting messages to a collector. Telemetry provides information about the network traffic, such as congestion, latency, and port buffer utlization, on a per-device basis. Monitoring provides insights about packet drops, and flow statistics.

### 4.1 Telemetry
Inband Telemetry (INT) is a variant of telemetry that provides insights into the path taken by the packets through the network, including per-hop latency, queue depth, and other metrics. Inband Flow Analyzer (IFA) is a mechanism INT uses to embed the telemetry data directly into the live data packet as they traverse the network. This mechanism allows for the precise tracking of flow paths and the calculation of residence time at each node.

The IFA architecture utilizes a distributed processing model across the network fabric, where each node performs a specialized function as packets traverse the data plane.

![tam_ifa_architecture](tam_ifa_arch.png)

In an IFA-aware network, the hops can perform one of the three roles:
1. **Initiator Node**: This node initiates the IFA packet and can operate in one of two modes:
   - **Inband Mode**: IFA header and the first IFA metadata with telemetry data is added to the data packet.
   - **Probe Mode**: Incoming data packets are sampled and the first IFA metadata is added to the sampled packet.
2. **Transit Node**: Matches on IFA packet and adds new IFA metadata to the packet.
3. **Terminator Node**: Inband IFA packet are stripped off the IFA header and metadata, and the datapacket is forwarded to the next hop, while the metadata is exported to a collector. Probe IFA packets are teminated and mirrored to a collector.

### 4.1.1 IFA2 packet format and encapsulation
The architecture supports the IFA2 specification, characterized by specific header structures and protocol identifiers:

![tam_ifa_packet_format](tam_ifa_packet_format.png)

- **Layer3 Encapsulation**: The L3 header utilizes IP protocol type 253 to identify IFA2 traffic.
- **Layer 4 Encapsulation**: The original Layer 4 header of the packet where the IFA header was added. 
- **IFA Header (4 bytes)**:
   - Version (4-bits): 2 for IFA2
   - Flags (8-bits): 
```
R | R | R | MF | TS | IB | TA | C
...
TS - TimeStamp format
IB - Inband - MUST be set for IFA2 packets
TA - Turn Around
C - Checksum 
```

![tam_ifa_header_format](tam_ifa_header_format.png)

- **IFA Metadata Header (4 bytes)**: Includes a Hop Limit (default 64) and a Current Length field, both of which are updated by transit nodes.

![tam_ifa_metadata_header_format](tam_ifa_metadata_header_format.png)

- **IFA Metadata Stack (32 bytes)**: Stores the actual telemetry data, including Device ID, Ingress/Egress port numbers, and high-precision timing data. Because each transit node appends a new metadata record to the packet, the total frame size grows incrementally as it traverses the network path.

![tam_ifa_metadata_stack_format](tam_ifa_metadata_stack_format.png)
...
2. **Stateful DM**: Drop reasons are exported on a per-packet,per-flow basis, in periodic intervals.

### 4.3 Comparison with existing mechanisms
The end goal of TAM maybe similar to other mechanisms that currently exist in SONiC, but there are differences in the approach. 
1. **sFlow**: sFlow uses a sampling mechanism to collect flow statistics. It requires the packet to be punt'ed to the CPU, which adds additional latency and overhead. TAM aims to provide a dataplane based solution for exporting telemetry and monitoring data directly from the ASIC.
2. **High Frequency Telemetry (HFT)**: HFT uses a polling mechanism to collect counter data. It requires the packet to be punt'ed to the CPU, which adds additional latency and overhead. TAM aims to provide a dataplane based solution for exporting telemetry and monitoring data directly from the ASIC. The data expored by HFT includes standard port counters, but does not include queue/buffer specific counters. TAM aims to provide a solution to export queue/buffer specific counters as well.
3. **DTEL**: DTel uses a trapping mechanism to collect telemetry data. The SAI implementation of DTEL predates the TAM proposal, and is not as flexible in terms of the types of data that can be exported. TAM aims to provide a more flexible framework that can be used to export a wider range of telemetry and monitoring data.
...
- Support Inband Probe mode for INT/IFA
...
; field             = value
SRC_IP              = IPv4address / IPv6address      ; mandatory
