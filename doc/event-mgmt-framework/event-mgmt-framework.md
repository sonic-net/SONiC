...
| **Term**       | **Meaning**              |
|----------------|--------------------------|
| EHT            | Event History Table      |
| CAT            | Current Alarm Table      |
...
1.  Event History Table
    ...
    Both stateful and stateless events get recorded in the Event History Table (EHT). EHT contains history of all events generated by the system.
    EHT is persisted across system restarts of any kind, including restore to factory defaults and SW upgrades and downgrades.

2. Current Alarm Table

   All raised stateful events - alarms - get recorded in Current Alarm Table (CAT). When a component that raised the alarm clears it, the alarm record is removed from CAT.
   An user acknowledging a particular alarm will NOT remove that alarm record from CAT.
   
   In effect, CAT contains outstanding alarms that need to be cleared by those components who raised them.
   CAT is NOT persisted and its contents are cleared with a reload.
   
In summary, the framework provides both current and historical event status of software and physical entities of the system through CAT and EHT. 
...
EHT should support 80k/30-day records. Considering the average size of an event to be 400 bytes, the total memory footprint of event history is ~4MB. Current active alarms are limited to total instances of unique alarms (which makes Current Alarm Table is a finite quantity)
...
Both EHT and CAT are stored under state-db.
...
- It verifies if the event is stateful. If so, it forwards the event to alarm consumer for further processing.
    - If state is raised, add to CAT
    - If state is clear, remove the entry from CAT
    - Update LED 
...
instances flood the EHT or CAT, eventd maintains a cache of last event/alarm. Every new event/alarm is compared against this cache entry
...
An example of an event in EHT raised by tam.
```
Event History Table:
==============================
...
The alarm record in the CAT is marked with is_acknowledged field set to true.

#### 3.3.2.2 Configuration Commands
These are implemented as RPCs.
```
sonic# event severity-profile <profile-name>
sonic# logging server <ip> [log|event] 
```
...
'show event' commands would display all the records in EHT.
...
'show alarm' command would display all the active alarm records in CAT.
...
# 7 Unit Test
- Raise an event and verify the fields in EHT and EVENT_STATS
- Raise an alarm and verify the fields in CAT and ALARM_STATS
- Clear an alarm and verify that record is removed from CAT and EVENT_STATS is udpated
- Ack an alarm and verify that is_acknowledged flag is set to true in CAT
- Verify wrap around for EHT ( change manifest file to a lower range and trigger that many events )
