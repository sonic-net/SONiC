...
The FEATURE table controls feature lifecycle. Key field:

```json
"set_owner": "local" | "kube"
```

- `local`: Managed by native `systemd`. FEATURE state drives container lifecycle.
- `kube`: Managed by Kubernetes. FEATURE table mirrors container state.
...
```bash
redis-cli -n 6 HSET "FEATURE|bmp" set_owner "kube"
```
...
4. **Update FEATURE Table**
   ```bash
   redis-cli -n 6 HSET "FEATURE|bmp" set_owner "local"
   redis-cli -n 6 HSET "FEATURE|bmp" enabled "True"
   ```
