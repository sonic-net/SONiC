...
| The list of OLT plugs currently registered on this PON Controller | /PON_CONTROLLER_SYSTEM_STATUS_OLT/{controller-name}\|{mac-address} |
...
| Use case | path |
|---|---|
| List of the NNI Networks configured on the OLT | /PON_OLT_INTF_NETWORK_STATE/{olt-intf-name}\|{vlan-id} |
| The MAC address learning table for this uplink port | /PON_OLT_INTF_NNI_NETWORK_LEARNING_TABLE_STATE/{olt-intf-name}\|{network-id}\|{mac-address} |
| The list of ONUs currently performing firmware upgrades on the OLT | /PON_OLT_INTF_ONU_FW_UPGRADE_STATE/{olt-intf-name}\|{onu-serial-number} |
| Consolidated list of ONUs with their operational state | /PON_OLT_INTF_ONU_OPERATIONAL_STATE/{olt-intf-name}\|{id} |
| List of OLT GEM ports | /PON_OLT_INTF_ONU_SERVICE_GEMPORT_STATE/{olt-intf-name}\|{onu-serial-number}\|{service-port-id} |
| List of OLT Service ports | /PON_OLT_INTF_ONU_SERVICE_TCONT_STATE/{olt-intf-name}\|{onu-serial-number}\|{service-port-id} |
| ONU related status | /PON_OLT_INTF_ONU_STATE/{olt-intf-name}\|{onu-serial-number} |
| List of OLT interfaces | /PON_OLT_INTF_STATE/{olt-intf-name} |
| List of OLT PON protection groups | /PON_OLT_PROTECTION_GROUP_STATE/{group-name} |
...
| Mapping entry that classifies VLAN CoS bit values to a destination GPON XGEM Port (OLT-... | /PON_ONU_OLT_SERVICE_DOWNSTREAM_QOS_MAP_MAP_STATE/{onu-name}\|{olt-service-id}\|{priority} |
| Downstream QoS Map | /PON_ONU_OLT_SERVICE_DOWNSTREAM_QOS_MAP_STATE/{onu-name}\|{olt-service-id} |
| List of C-VLAN IDs through the service flow List of PON VLAN network through service flow | /PON_ONU_OLT_SERVICE_NETWORK_STATE/{onu-name}\|{olt-service-id}\|{vlan-id} |
...
| OLT plug temperatures | /PON_OLT_STATISTICS_ACCUMULATING_TEMP/{olt-name} |
...
| OLT plug NNI port statistics | /PON_OLT_PLUG_STATISTICS_BINNED_NNI/{olt-name}\|{olt-stats-id} |
| OLT plug device temperatures | /PON_OLT_PLUG_STATISTICS_BINNED_TEMP/{olt-name}\|{olt-stats-id} |

### ONU OLT-side Statistics

| Use case | path |
|---|---|
| List containing the statistics history for this device | /PON_OLT_INTF_STATISTICS_BINNED/{olt-intf-name}\|{olt-stats-id} |
