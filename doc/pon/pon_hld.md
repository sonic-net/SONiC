...
| config    | `pon olt-intf <name>` → `device-id <id>` / `pon-enable` / ...    | CONFIG_DB `PON_OLT_INTF\|<olt-intf-name>\|<port-id>`          |
...
| exec      | `show pon olt-intf [<name>]`                                                                | CONFIG_DB `PON_OLT_INTF\|<olt-intf-name>\|<port-id>`          |
...
| UT-02 | `PonOrch` | APPL_DB SET on `PON_OLT_INTF_TABLE:<olt-intf-name>\|<port-id>` after controller exists                          | `sai_create_pon_olt_fn` invoked with `SAI_PON_OLT_ATTR_CONTROLLER_ID` set to the controller OID.                |
| UT-03 | `PonOrch` | APPL_DB SET on `PON_OLT_INTF_TABLE:<olt-intf-name>\|<port-id>` before parent controller exists                 | Operation deferred and retried after the parent appears; no SAI create issued out of order.                     |
...
| UT-05 | `PonOrch` | APPL_DB DEL on `PON_OLT_INTF_TABLE:<olt-intf-name>\|<port-id>` while child ONU rows still exist                | Children removed first (or remove deferred until children removed); no orphaned ONU SAI objects.                |
| UT-06 | `PonOrch` | APPL_DB attribute-only SET on `PON_OLT_INTF_TABLE:<olt-intf-name>\|<port-id>` (e.g. `pon-enable` toggled)     | `sai_set_pon_olt_attribute_fn` invoked once with the changed attribute; no create / remove issued.              |
...
| IT-02 | CONFIG_DB → APPL_DB                    | Operator writes `PON_OLT_INTF\|<olt-intf-name>\|<port-id>` then `PON_ONU\|<name>`                                | `PON_OLT_INTF_TABLE` and `PON_ONU_TABLE` appear in APPL_DB with parent references resolved.       |
